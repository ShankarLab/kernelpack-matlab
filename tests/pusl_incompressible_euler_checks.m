function pusl_incompressible_euler_checks()
%PUSL_INCOMPRESSIBLE_EULER_CHECKS Targeted manufactured check for the PU-SL Euler port.
...
p_rel = relative_l2_vec(p, p_exact);

assert(isfinite(u_rel) && isfinite(p_rel), 'PUSL incompressible Euler should produce finite errors.');
assert(u_rel < 3.0e-1, 'PUSL incompressible Euler velocity error is too large.');
assert(p_rel < 6.0e-1, 'PUSL incompressible Euler pressure error is too large.');
assert(sol.divergence_rms < 5.0e-1 && sol.divergence_max < 2.0, 'Divergence diagnostics are too large.');
assert(sol.wall_normal_rms < 5.0e-2 && sol.wall_normal_max < 2.0e-1, 'Slip-wall diagnostics are too large.');

disp('pusl incompressible euler checks passed');
end
