function results = pusl_advection_rotation_convergence_2d()
%PUSL_ADVECTION_ROTATION_CONVERGENCE_2D Rigid-rotation convergence study for PU-SL.
...
for io = 1:numel(orders)
    ...
    for ih = 1:numel(hValues)
        ...
        c = solver.projectInitial(@(x) initial_value_row(x));
...
function values = initial_value_row(x)
values = exp(0.3 * x(1) - 0.2 * x(2)) + sin(0.7 * x(1) + 0.4 * x(2));
end

function values = initial_value(X)
values = exp(0.3 * X(:, 1) - 0.2 * X(:, 2)) + sin(0.7 * X(:, 1) + 0.4 * X(:, 2));
end
...
fprintf('\n2D rigid-rotation PU-SL advection convergence study\n');
